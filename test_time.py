...
class RovLogger:
    ...
    def __init__(self, config):
        ...
        # название файла
        name = config['path_log'] + '-'.join(
            '-'.join('-'.join(str(datetime.now()).split()).split('.')).split(':')) + '.log'
        ...
        coloredlogs.install(level=log_level[config['log_level']],
                            logger=self.logs, fmt='%(asctime)s : %(levelname)s : %(message)s')
...
if __name__ == '__main__':
    config_log = {'log_level': 'debug',
                  'path_log': ''}
    logger = RovLogger(config_log)
    logger.info('Start test time')
    while True:
