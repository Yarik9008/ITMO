import sys
import pickle
### обработка многостроченого текста невозможка ###
...
def decode_huffman(path_input: str, path_output: str):
    with open(path_input) as file_in:
        ...
        for i in range(int(data[0])):
            sim, code = data[i + 1][1:-1].split('"-"')
            mass_key.append([sim, code])
            
        mass_key = sorted(mass_key, key=lambda col: len(col[1]))
        
        text_in  = data[int(data[0]) + 1]
        text_out = ''
